...
namespace MSR.LST.RTDocuments.Utilities
{
    ...
    public class PPTConversion
    {
        ...
        private static string GetPresentationProperty(XpsDocument pres, string propertyName)
        {
            // Get the Document Properties
            object oDocBuiltInProps = pres.CoreDocumentProperties;
        ...
        //private static string GetSlideTitle(DocumentPage s)
        //{
        //    // If the slide has a Title shape, use that text
        //    if (s.Visual. == Microsoft.Office.Core.MsoTriState.msoTrue)
        ...
        private static Image GetSlideImage(DocumentPage docPage, MemoryStream ms)
        {
            ...
            string jpgfilename = tfc.BasePath + "\\" + Guid.NewGuid().ToString() + ".jpg";
            using (FileStream pageOutStream = new FileStream(jpgfilename, FileMode.Create, FileAccess.Write))
            {
